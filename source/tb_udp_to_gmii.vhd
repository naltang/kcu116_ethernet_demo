...
architecture sim of tb_udp_to_gmii is
    ...
    signal verified_frames : natural range 0 to 2 := 0;
    ...
    constant SOURCE_MAC_OFFSET : natural := 14;
    constant SOURCE_IP_OFFSET  : natural := 34;
    constant DEST_IP_OFFSET    : natural := 38;
    constant UDP_OFFSET        : natural := 42;
    constant PAYLOAD_OFFSET    : natural := 50;

    type byte_array_t is array (natural range <>) of std_logic_vector(7 downto 0);

    procedure verify_frame(constant captured : in byte_array_t) is
    ...
    begin
        ...
        for current_byte in 8 to 13 loop
            assert captured(current_byte) = x"FF"
                report "Incorrect destination MAC address" severity failure;
        end loop;
        for current_byte in 0 to 5 loop
            assert captured(SOURCE_MAC_OFFSET + current_byte) =
        ...
        for current_byte in 0 to 3 loop
            assert captured(SOURCE_IP_OFFSET + current_byte) =
                TEST_SOURCE_IP(31 - current_byte * 8 downto
                               24 - current_byte * 8)
                report "Incorrect source IPv4 address" severity failure;
            assert captured(DEST_IP_OFFSET + current_byte) =
                TEST_DESTINATION_IP(31 - current_byte * 8 downto
                                    24 - current_byte * 8)
                report "Incorrect destination IPv4 address" severity failure;
        end loop;
        assert captured(UDP_OFFSET) = x"27" and
               captured(UDP_OFFSET + 1) = x"10"
            report "Incorrect UDP source port" severity failure;
        assert captured(UDP_OFFSET + 2) = x"4E" and
               captured(UDP_OFFSET + 3) = x"20"
            report "Incorrect UDP destination port" severity failure;
        assert captured(UDP_OFFSET + 4) = x"00" and
               captured(UDP_OFFSET + 5) = x"0D"
            report "Incorrect UDP length" severity failure;

        for current_byte in 0 to PAYLOAD_BYTE_COUNT - 1 loop
            assert captured(PAYLOAD_OFFSET + current_byte) =
                TEST_PAYLOAD(
        ...
        for current_byte in
            PAYLOAD_OFFSET + PAYLOAD_BYTE_COUNT to
            PREAMBLE_LENGTH + FRAME_DATA_LENGTH - 1
        loop
        ...
        for current_byte in 22 to 40 loop
            if current_byte mod 2 = 0 then
                word_value(15 downto 8) := unsigned(captured(current_byte));
        ...
        while checksum_sum > 65535 loop
            checksum_sum := checksum_sum mod 65536 + checksum_sum / 65536;
        end loop;
        assert checksum_sum = 65535
            report "Incorrect IPv4 header checksum" severity failure;

        checksum_sum := 17;
        for current_byte in SOURCE_IP_OFFSET to UDP_OFFSET - 2 loop
            if current_byte mod 2 = 0 then
                word_value(15 downto 8) := unsigned(captured(current_byte));
                word_value(7 downto 0) :=
                    unsigned(captured(current_byte + 1));
        ...
        word_value(15 downto 8) := unsigned(captured(UDP_OFFSET + 4));
        word_value(7 downto 0) := unsigned(captured(UDP_OFFSET + 5));
        checksum_sum := checksum_sum + to_integer(word_value);

        for word_index in 0 to (UDP_LENGTH + 1) / 2 - 1 loop
            byte_index := UDP_OFFSET + word_index * 2;
            word_value(15 downto 8) := unsigned(captured(byte_index));
            if byte_index + 1 < UDP_OFFSET + UDP_LENGTH then
                word_value(7 downto 0) :=
        ...
        crc := (others => '1');
        for current_byte in
            PREAMBLE_LENGTH to PREAMBLE_LENGTH + FRAME_DATA_LENGTH - 1
        loop
...
begin
    ...
    begin
        ...
        wait until verified_frames = 2;
        report "Two deterministic UDP-to-GMII frames verified"
            severity note;
        stop;
        wait;
    ...
    begin
        wait for 25 us;
        assert false report "UDP-to-GMII test timed out" severity failure;
        wait;
